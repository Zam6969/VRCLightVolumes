...
#region Shader Buffer Rebuild And Upload
        ...
        private void WritePointLightShaderData(int shaderIndex, int sourceIndex, PointLightVolumeInstance instance, Transform instanceTransform, bool countActiveShadow) {
            ...
            float areaShapeSpread = instance.AreaLightShapeSpread == instance.AreaLightShapeSpread ? Mathf.Clamp01(instance.AreaLightShapeSpread) : 1f;
            _pointLightAreaTriangleData[areaTriangleIndex] = new Vector4(triangleA.x, triangleA.y, triangleB.x, triangleB.y);
            _pointLightAreaTriangleData[areaTriangleIndex + 1] = new Vector4(triangleC.x, triangleC.y, triangleAreaScale, areaShapeSpread + 1f);
            if (hasShadow) {
